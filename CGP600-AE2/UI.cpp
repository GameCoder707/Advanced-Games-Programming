...
UI::UI(string filename, ID3D11Device* device, ID3D11DeviceContext* context)
{
	pD3DDevice = device;
...
void UI::AddText(string s, float x, float y, float size)
{ 
	string_2d temp; 
	temp.s = s ;
	temp.x = x; 
...
void UI::RenderText(void)
{
	...
	for(int i=0; i < s2d.size(); i++)
	{
		...
		for(int j = 0;j < s2d[i].s.length(); j++)
		{
			...
			else if(c >= '/' && c <= ':') // numbers and few symbols
			{
